...
const getJSONConfig = async (workspace, path, filename) => {
	const filePath = join(workspace || '.', path, filename)
	const content = await readFile(filePath, 'utf-8')
	return JSON.parse(content)
}

export default async ({ github, context, core, workspace }) => {
	try {
		const config = await getJSONConfig(workspace, 'config', 'issue-triage.json')
