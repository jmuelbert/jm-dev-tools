...
#######################################
generate_guide() {
    local types_joined=$(IFS=', '; echo "${PROJECT_TYPES[*]}")
    ...
    local python_pm_tool=$(detect_python_pm)
    local node_pm="npm"
    ...
    if [[ $is_python -eq 1 ]]; then
        ...
        if [[ "$python_pm_tool" == "Poetry" ]]; then pm_ref="Poetry"; install_cmd="poetry install --with dev"; fi
        if [[ "$python_pm_tool" == "Hatch" ]]; then pm_ref="Hatch"; install_cmd="hatch env create"; fi
        if [[ "$python_pm_tool" == "PDM" ]]; then pm_ref="PDM"; install_cmd="pdm install"; fi
    ...
    if [[ $task_runner_found -eq 1 ]]; then
        ...
        if [[ $is_python -eq 1 ]]; then
            ...
            echo "\`\`\`bash"
            if [[ "$python_pm_tool" == "Hatch" ]]; then echo "hatch run default: python -m project_name";
            elif [[ "$python_pm_tool" == "Poetry" ]]; then echo "poetry run python -m project_name";
            elif [[ "$python_pm_tool" == "PDM" ]]; then echo "pdm run start";
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            *) PROJECT_DIR="$1"; shift ;; # Positional argument is directory
        esac
    done
